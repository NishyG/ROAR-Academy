...
while True:
    ...
    hour_vector = clock_hand_vector((hour/12*2*np.pi)+((minute/60*2*np.pi)+((second/60*2*np.pi)/60))/12, hour_hand_length)
    ...
    gmt_vector = clock_hand_vector(((gmt/24*2*np.pi)+(((hour/12*2*np.pi)+((minute/60*2*np.pi)+((second/60*2*np.pi)/60))/12)/2)) + 7, hour_hand_length)
